...
// the data from the file is stored in both of the maps given as parameters.
// Returns false if the file cannot be opened, other returns true.
bool save_data(const string& file_name,
               map<string, shared_ptr<Student>>& numerical_order) {
    ...
    map<string, shared_ptr<Student>>::iterator iter = numerical_order.begin();

    while( iter != numerical_order.end() )  {
        file_object << iter->second.get()->student_number << ";" << iter->second.get()->user_id << ";"
            << iter->second.get()->name << ";" << iter->second.get()->phone_number << ";" << iter->second.get()->email
            << ";" << iter->second.get()->skype << endl ;
        iter++;
    }
    file_object.close();
    return true;
    ...
    while( true ) {
        ...
        // Changing student data
            ...
            if( parts.size() != 2 ){
             ...
             map<string, shared_ptr<Student>>::iterator iter = student_numbers.find(parts.at(1));
